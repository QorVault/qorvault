"""Database access for the minutes fact layer.
...
shell before running any script in this package::
...
def connect(readonly: bool = False) -> Iterator[Any]:
    ...
    Args:
        readonly: Open the session read-only. Use for every query path that
            ...
            it makes the read-only contract enforced rather than assumed.
...
def query(sql: str, params: tuple | None = None) -> list[tuple]:
    ...
    The session is opened read-only, so an accidental write against
    ``documents`` or ``chunks`` fails at the database rather than silently
    succeeding.
